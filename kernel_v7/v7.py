...
if 'PitNextLap' not in orig.columns:
    ...
    print(f'Derived PitNextLap for orig: {orig["PitNextLap"].mean():.4f} rate')
...
print(f'Combined train: {tr_combined.shape}')
...
def build_features(tr_df, te_df):
    ...
    # Target encoding (full train for simplicity - will do KFold-aware in model loop)
...
def run_xgb_v7(name, params, seed):
    oof = np.zeros(len(y_synth), dtype=np.float64)
    test_pred = np.zeros(len(X_te), dtype=np.float64)
    p = dict(params); p['objective']='binary:logistic'; p['eval_metric']='auc'
    p['tree_method']='hist'; p['nthread']=NTHREADS; p['seed']=seed; p['verbosity']=0
    print(f'\n[{name}] XGB 5-fold (synth+orig)...')
    for f in range(5):
        tr_mask = folds != f; va_mask = folds == f
        X_fold_tr = np.vstack([X_synth[tr_mask], X_orig])
        y_fold_tr = np.concatenate([y_synth[tr_mask], y_orig])
...
oof1, t1, _ = run_lgb_v7('LGB1', dict(num_leaves=127, learning_rate=0.03, min_data_in_leaf=200,
    feature_fraction=0.8, bagging_fraction=0.8, bagging_freq=5, lambda_l2=1.0), 42)
oof2, t2, _ = run_lgb_v7('LGB2', dict(num_leaves=255, learning_rate=0.03, min_data_in_leaf=150,
    feature_fraction=0.75, bagging_fraction=0.85, bagging_freq=5, lambda_l2=1.5), 2026)
oof3, t3, _ = run_xgb_v7('XGB', dict(max_depth=8, eta=0.03, subsample=0.8,
    colsample_bytree=0.7, reg_lambda=1.0, reg_alpha=0.1), 2026)
oof4, t4, _ = run_lgb_v7('DART', dict(boosting='dart', num_leaves=63, learning_rate=0.05,
    min_data_in_leaf=200, feature_fraction=0.7, bagging_fraction=0.8, bagging_freq=5,
    lambda_l2=1.0, drop_rate=0.1, skip_drop=0.5, max_drop=50), 11)

print('\n=== Blend ===')
names = ['lgb1','lgb2','xgb','dart']
oofs = [oof1, oof2, oof3, oof4]
tests = [t1, t2, t3, t4]
...
blend_oof = oof_mat @ w
print(f'BLEND OOF AUC = {roc_auc_score(y_synth, blend_oof):.6f}')

pd.DataFrame({'id': test_ids, 'PitNextLap': blend_test}).to_csv(f'{WD}/submission_v7.csv', index=False)
