...
device = torch.device('cpu')  # force CPU — Kaggle GPU has CUDA arch mismatch
...
print('\n=== BiLSTM (CPU, O(1) lookups) ===')
lstm_oof, lstm_test = train_model(BiLSTM, dict(input_dim=N_FEAT, hidden=64, layers=2, dropout=0.3),
                                   n_epochs=15, lr=2e-3, bs=128)

print('\n=== 1D-CNN (CPU, O(1) lookups) ===')
cnn_oof, cnn_test = train_model(CNN1D, dict(input_dim=N_FEAT, hidden=64),
                                 n_epochs=15, lr=2e-3, bs=128)
